...
export const TrialTagsService = {
  listTags: (userId) => TrialTagsRepository.findAll(prisma, userId),

  createTag: async (userId, payload) => {
    const parsed = createSchema.parse(payload);
    const existing = await TrialTagsRepository.findByName(prisma, userId, parsed.name);
    if (existing) {
      throw new HttpError(409, 'Тег с таким именем уже существует');
    }
    const id = generateId('trialtag');
    await TrialTagsRepository.insert(prisma, userId, {
      id,
      name: parsed.name,
      color: parsed.color ?? null,
    });
    return TrialTagsRepository.findById(prisma, userId, id);
  },
  ...
  updateTag: async (userId, id, payload) => {
    ...
    if (data.name) {
      const dup = await TrialTagsRepository.findByName(prisma, userId, data.name);
      if (dup && dup.id !== id) {
        throw new HttpError(409, 'Тег с таким именем уже существует');
      }
    }
    const result = await TrialTagsRepository.updateById(prisma, userId, id, data);
    if (result.count === 0) {
  ...
  deleteTag: async (userId, id) => {
    const result = await TrialTagsRepository.deleteById(prisma, userId, id);
    if (result.count === 0) {
      throw new HttpError(404, 'Тег не найден');
    }
